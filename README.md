...
- [x] Well-defined AI/ML project using **Endee** as the vector database  
- [x] Practical use case: **semantic search** and **RAG**  
- [x] Hosted on GitHub  
- [x] README with project overview, problem statement, system design, how Endee is used, and setup/execution instructions  

---

## What You Can Do Next

- Add more documents under `data/` or point `--path` to your own file.
- Use metadata filters in `query_vectors()` (e.g. by `doc_index` or custom tags) for filtered search.
- Swap the embedding model in `config.py` (and dimension) for higher quality or different languages.
- Integrate other LLM providers (Ollama, Anthropic, etc.) in `query.py` for RAG answers.
